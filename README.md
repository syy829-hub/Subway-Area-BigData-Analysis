...
항목	        사용 기술	               역할
수집(Ingest)	Python (Requests)	       공공데이터 API 호출 및 수집 스크립트 자동화
저장(Storage)	   HDFS	                   수집된 100MB 이상의 정형 데이터 적재
처리(Processing)	Apache Hive	         대규모 데이터셋 스키마 정의 및 데이터 조인(Join)
분석(Analysis)	  Apache Spark	       유동인구와 상권 밀집도 간의 상관관계 통계 분석
시각화(Viz)	  Seaborn / Matplotlib	   분석 결과(혼잡도 리포트) 그래프 도출
3. 구현 계획 (Pipeline)
